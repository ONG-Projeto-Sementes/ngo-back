...
export class AnalyticsService {
    ...
    async getDashboardOverview(filters = {}) {
        ...
        const [totalStats, distributionStats, familyStats, recentDonations, topDonations, statusBreakdown, categoryBreakdown] = await Promise.all([
    ...
    async getCategoryPerformance(filters = {}) {
        ...
        return {
            ...
            summary: {
                ...
                mostEfficientCategory: categoryPerformance.reduce((prev, current) => (current.efficiency > prev.efficiency) ? current : prev, categoryPerformance[0] || {})?.categoryName || 'N/A'
            },
            generatedAt: new Date()
        };
    }
     ...
     */
    buildDateFilter(filters, dateField = 'receivedDate') {
        let dateFilter = {};
     ...
     */
    getGroupFormat(groupBy, dateField = '$receivedDate') {
     ...
     */
    async getTotalDonationStats(filters) {
     ...
     */
    async getDonationStatusBreakdown(filters) {
     ...
     * Análise de retenção de doadores
     */
    async getDonorRetentionAnalysis(dateFilter) {
     ...
     * Eficiência por categoria
     */
    async getCategoryEfficiency(dateFilter) {
        return await DonationModel.aggregate([
            { $match: { deleted: false, ...dateFilter } },
            {
                $lookup: {
     ...
     */
    async getPendingDistributionsCount() {
